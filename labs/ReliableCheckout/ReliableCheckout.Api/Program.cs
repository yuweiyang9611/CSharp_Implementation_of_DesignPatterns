...
var administrative = args.FirstOrDefault() == "outbox";
var app = await CheckoutApplication.CreateAsync(administrative ? [] : args);
if (administrative)
{
    if (args.Length == 2 && args[1] == "list")
        Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(await app.Services.GetRequiredService<CheckoutStore>().GetOutboxAsync()));
    else if (args.Length == 3 && args[1] == "replay" && Guid.TryParse(args[2], out var eventId))
    {
        if (!await app.Services.GetRequiredService<OutboxLeases>().ReplayAsync(eventId))
        { Console.Error.WriteLine("Only an existing dead-letter event can be replayed."); Environment.ExitCode = 2; }
    }
    else { Console.Error.WriteLine("Usage: outbox list | outbox replay <eventId>"); Environment.ExitCode = 2; }
...
public static class CheckoutApplication
{
    public static async Task<WebApplication> CreateAsync(string[] args, Action<WebApplicationBuilder>? configure = null)
    {
        ...
        builder.Services.AddSingleton<PersistentPaymentSdk>();
        builder.Services.AddSingleton<ILegacyPaymentSdk, DurableLegacyPaymentSdk>();
        builder.Services.AddSingleton<IPaymentGateway, CallbackPaymentGatewayAdapter>();
        builder.Services.AddSingleton<ReservationService>();
        builder.Services.AddSingleton<OutboxLeases>();
        builder.Services.AddSingleton<IOutboxHandler, CancelPaymentHandler>();
        builder.Services.AddSingleton<IOutboxHandler>(services => new OrderProjectionHandler(
            "OrderSettled", services.GetRequiredService<CheckoutDatabase>(), services.GetRequiredService<IClock>(),
